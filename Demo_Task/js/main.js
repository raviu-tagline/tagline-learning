...
/*function sort_order()
{
    $("table > tbody").each(function (ind, vals) {
        $("tbody > tr").each(function (ind, vals){
            var cur = $("tbody").find("#name" + ind + " td").eq(1).text()
            var next = ind + 2
            var nex = $("tbody").find("#name" + next + " td").eq(1).text();
            console.log("Current: ", cur, " next: ", nex);
        })
        // var tData = $(this).find('td').eq(1).text();
        // var stdata = $(this).find
        // console.log(tData);
    });
}*/
